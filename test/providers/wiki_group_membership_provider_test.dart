...
void main() {
  group('wikiGroupMembershipProvider', () {
    ...
    test('returns null when no user is signed in', () async {
      ...
      final result = await waitForProvider(container, wikiGroupMembershipProvider);
    ...
    test('idempotent: calling twice does not error', () async {
      ...
      final hash1 = await waitForProvider(container, wikiGroupMembershipProvider);
      // Invalidate and re-read to simulate rebuild
      container.invalidate(wikiGroupMembershipProvider);
      final hash2 = await waitForProvider(container, wikiGroupMembershipProvider);
  ...
  group('normalizeWikiUrl', () {
    test('trims whitespace', () {
      expect(normalizeWikiUrl('  https://wiki.example.com  '),
          'wiki.example.com');
    });
    ...
    test('strips http scheme', () {
      expect(normalizeWikiUrl('http://wiki.example.com'),
          'wiki.example.com');
    });
